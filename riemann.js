...
function step(){
  ...
  x0 =canvas.width/2+my_zeta(sj).re*(.8*canvas.width)/8;
    y0 = canvas.height/2-my_zeta(sj).im*(.8*canvas.height)/6;
    ctx.moveTo(x0, y0);
    sj = math.complex(x_value,(i+1)/30);
    x1 =canvas.width/2+my_zeta(sj).re*(.8*canvas.width)/8;;
    y1 = canvas.height/2-my_zeta(sj).im*(.8*canvas.height)/6;  
}
ctx.lineTo(x1, y1);
ctx.stroke();
...
function my_zeta(z){
   var z = math.complex(z)
   console.log(z);
  var N = 10000;
  //var z = math.complex(x,y);
  var f1 = math.complex(0);
  console.log(f1);
  var f2 = math.complex(0);
  var zeta = math.complex(0);
  for (var k = 1; k < N; k++) {
f1 = math.divide((-1)**(k-1),math.pow(k,z));
zeta = math.add(zeta,f1);
}
console.log(f1);
f2 = math.subtract(1,math.pow(2,math.subtract(1,z)));
console.log(f2);
  zeta = math.divide(zeta,f2);
